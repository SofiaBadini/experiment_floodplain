"""This script uses the ``outcomes_flood.txt`` template and the data in ``interaction_flood_experience.csv``
...
depends_on = {
    "template": SRC / "final" / "tables_templates" / "outcomes_flood.txt",
    "data": BLD / "analysis"/ "heterogeneity" / "interaction_flood_experience.csv",
}
produces = BLD / "tables" / "outcomes_flood.tex"
...
def task_make_tables(depends_on, produces):
    ...
    vars = [
        ...
        "maps:julyflood_damages_yes", 
        "WTS:julyflood_damages_yes",
        "insurance:julyflood_damages_yes"
        ]
