"""This script uses the ``outcomes_belief_confidence.txt`` template and the data in ``interaction_belief_confidence.csv`` 
...
depends_on = {
    "template": SRC / "final" / "tables_templates" / "outcomes_belief_confidence.txt",
    "data": BLD / "analysis"/ "heterogeneity" / "interaction_belief_confidence.csv",
}
produces = BLD / "tables" / "outcomes_belief_confidence.tex"
...
def task_make_tables(depends_on, produces):
    ...
    frictions = ["risk_", "damages_", "comptot_", "compshare_"]
    new_dfs = []
    ...
    vars = [
        ...
        "insurance", 
        "conf_standardized", 
        "maps:conf_standardized", 
        "WTS:conf_standardized",
        "insurance:conf_standardized"
        ]
