"""This script uses the ``outcomes_wtp_alternative.txt`` template and the data in
...
depends_on = {
    "template": SRC / "final" / "tables_templates" / "outcomes_wtp_alternative.txt",
    "wtp_no_wins": BLD / "analysis" / "outcomes" / "wtp_no_wins.csv",
    "wtp_wins975": BLD / "analysis" / "outcomes" / "wtp_wins975.csv"
}
produces = BLD / "tables" / "outcomes_wtp_alternative.tex"
...
def task_make_tables(depends_on, produces):
    ...
    datadict = {}
    cols = ["wtp_no_wins", "wtp_wins975"]    
    treatments = ["maps", "WTS", "insurance"]

    reskeys = ["unadjusted", "covs", "post", "double"]
    resdict = {key: depends_on[key] for key in cols}

    # load data
    for col in cols:
        datadict[col] = (pd.read_csv(
            resdict[col], 
            sep=";", index_col=[0], 
            header=[0,1]).dropna())
